...
safe to remove; unlabeled torrents are never touched; removal always
unseeds without deleting files), and the shared queued-superseded filter.
...
def run():
    ...
    # h1: library-seed label, stale + negligible upload + plenty of swarm
    #     seeds -> unseeded (remove_data=False, file stays on disk)
    ...
    # h5: same as h1 but NO label at all (unmanaged) -> kept, never touched
    seed_torrents = {
        'h1': {'name': 'Safe To Remove', 'label': aw.LIBRARY_SEED_LABEL, 'state': 'Seeding',
               'seeding_time': 999 * 86400, 'total_uploaded': 1000, 'total_seeds': 6},
        'h2': {'name': 'Unknown Swarm', 'label': aw.LIBRARY_SEED_LABEL, 'state': 'Seeding',
               'seeding_time': 999 * 86400, 'total_uploaded': 1000, 'total_seeds': -1},
        'h3': {'name': 'Too Few Seeds', 'label': aw.LIBRARY_SEED_LABEL, 'state': 'Seeding',
               'seeding_time': 999 * 86400, 'total_uploaded': 1000, 'total_seeds': 5},
        'h4': {'name': 'No Seed Field', 'label': aw.LIBRARY_SEED_LABEL, 'state': 'Seeding',
               'seeding_time': 999 * 86400, 'total_uploaded': 1000},
        'h5': {'name': 'Unmanaged, No Label', 'label': '', 'state': 'Seeding',
               'seeding_time': 999 * 86400, 'total_uploaded': 1000, 'total_seeds': 6},
    }
    ...
    try:
        ...
        removed_calls = []
        aw.remove_torrent = lambda h, remove_data=True: removed_calls.append((h, remove_data))
        ...
        assert removed_calls == [], removed_calls

        # second pass (upload unchanged since baseline): gate decides
        _stub_deluge(aw, seed_torrents, set())
        candidates = aw.cleanup_stalled_seeds(dry_run=False)
        # only h1 qualifies, and it must be unseeded (files kept), not deleted
        assert removed_calls == [('h1', False)], removed_calls
