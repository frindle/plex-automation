...
LITERALS = [[None, 'def radarr_bulk_search():'], [None, 'def sonarr_bulk_search():'], [None, '_load_upgrade_state()'], [None, '_save_upgrade_state(state)'], [None, 'sort_ids_by_year_desc(monitored)'], [None, "advance_upgrade_cursor(entry.get('cursor', 0), len(movie_ids))"], [None, "advance_upgrade_cursor(entry.get('cursor', 0), len(series_ids))"], [None, "'name': 'MoviesSearch'"], [None, "'name': 'SeriesSearch'"], [None, "state['radarr'] = {'cursor': next_cursor, 'last_run': datetime.now(timezone.utc).isoformat()}"], [None, "state['sonarr'] = {'cursor': next_cursor, 'last_run': datetime.now(timezone.utc).isoformat()}"], [None, "@app.route('/run-bulk-search', methods=['POST'])"], [None, "'service must be radarr or sonarr'"]]
