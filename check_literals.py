...
TARGET = pathlib.Path('arr-webhook.py')

# frozen from TASK.md -- re-run `ollama-dispatch-scaffold --freeze-literals .`
LITERALS = [[None, '_is_recent_year'], [None, 'def _is_recent_year(year, now=None)'], [None, "RADARR_UPG_PRIORITY_LABEL = os.environ.get('RADARR_UPGRADE_PRIORITY_LABEL', 'radarr-upgrade-recent')"], [None, "SONARR_UPG_PRIORITY_LABEL = os.environ.get('SONARR_UPGRADE_PRIORITY_LABEL', 'sonarr-upgrade-recent')"], [None, "_is_recent_year(movie.get('year'))"], [None, 'ensure_label_exists_named(RADARR_UPG_PRIORITY_LABEL)'], [None, 'set_torrent_label(torrent_hash, RADARR_UPG_PRIORITY_LABEL)'], [None, 'priority_hashes.append(torrent_hash)'], [None, "'method': 'core.queue_top', 'params': [priority_hashes]"], [None, '_is_recent_year(air_year)'], [None, 'ensure_label_exists_named(SONARR_UPG_PRIORITY_LABEL)'], [None, 'set_torrent_label(torrent_hash, SONARR_UPG_PRIORITY_LABEL)'], [None, 'airDateUtc'], [None, "priority_labels = {'sonarr', 'radarr', SONARR_UPG_PRIORITY_LABEL, RADARR_UPG_PRIORITY_LABEL}"]]
