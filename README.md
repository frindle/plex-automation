...
- Add `media_share.py`: friend-facing media portal at `/share`, gated by Cloudflare Access identity — browse Movies/TV/Music libraries, download individual files (HTTP Range-resumable) or push a file/folder to a per-friend SFTP destination (`FRIENDS_CONFIG`), rate-limited to 5 Mbit/s; SQLite-backed per-friend usage tracking at `/share/usage`; clean dark-theme UI.
