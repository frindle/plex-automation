...
- **Daily**: removes `superseded` torrents that have seeded past `SEED_DAYS` (default 21), and dedupes Radarr's downloads (keeps the highest custom-format-score entry per movie, across both Radarr's own queue and the throttled `radarr-upgrade` lane, which Radarr's queue cannot see).
