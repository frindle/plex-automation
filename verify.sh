#!/usr/bin/env bash
# verify for: arr-webhook-recent-upgrade-priority-s4-hourly-sweep-keeps-recent-on-top
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('arr-webhook.py').read())"; then
  echo "  ok: arr-webhook.py parses"
else
  echo "  FAIL: arr-webhook.py does not parse"; fails=$((fails+1))
fi
